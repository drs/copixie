...
class CoPixie():
    ...
    def main(self):
        ...
        experiment = self._create_experiment_from_metadata(config)
        self.logger.info("Loaded the experiment from the metadata file (found {} assays).".format(len(experiment)))

        self._create_output_dir()
        cells = experiment.get_cells()

        if not cells:
            self.logger.error("No cells found in the filestructure.")
            sys.exit(1)
    ...
    def _get_arguments(self):
        ...
        parser.add_argument('-o', '--output', type=str, default="DCTracker-{}".format(datetime.datetime.now().strftime("%Y-%m-%d_%H%M%S")), 
    ...
    def _create_experiment_from_metadata(self, config):
        """create an experiment from the metadata"""

        experiment = None
        try:
            experiment = Experiment().from_file(self.metadata_file, config)
        except RuntimeError as e:
            self.logger.error(e)
            sys.exit(1)
